...
RSpec.describe Truelist::Configuration do
  ...
  describe '#api_key!' do
    ...
    it 'raises Truelist::AuthenticationError when api_key is nil' do
      config.api_key = nil
      expect { config.api_key! }.to raise_error(Truelist::AuthenticationError, /API key is not configured/)
    end
  end
